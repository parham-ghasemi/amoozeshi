...
const VideoCard = ({ video }: { video: VideoShort }) => {
  ...
  return (
    <Link to={`/video/${video._id}`} className="w-[290px] h-[282px] flex flex-col cursor-pointer rounded-xl hover:shadow-xl transition-shadow duration-200 group">
      ...
      <p className="text-lg font-medium mt-2.5 mb-3 line-clamp-2 pt-2.5 pr-2.5">{video.title}</p>
