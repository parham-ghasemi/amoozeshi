...
const CourseShowcase = () => {
  ...
  return (
    <div className="w-full max-w-[95%] lg:max-w-7xl mx-auto p-4 sm:p-6 md:p-8 space-y-6 flex flex-col items-center min-h-screen">
      <h1 className="w-full max-w-[90%] sm:max-w-lg md:max-w-2xl text-center text-xl sm:text-2xl md:text-3xl font-bold leading-[150%] break-words">
        {course.title}
      </h1>
      <div className="w-full flex flex-col md:flex-row gap-4 md:gap-6">
        {/* LEFT */}
        <div className="w-full md:w-lg flex flex-col gap-4 md:gap-6">
          <div className="bg-white shadow-xl w-full md:max-w-96 h-fit rounded-lg p-4 sm:p-5 flex flex-col gap-5 md:gap-7">
            <div className="w-full bg-blue-300 rounded-lg px-4 sm:px-7 py-3 sm:py-4 flex items-center gap-3 sm:gap-5 justify-end">
              <p className="flex text-nowrap gap-1.5 text-sm sm:text-base">
                <span>{course.joinedBy.length}</span>
                :تعداد دانشجویان
              </p>
            ...
            <p className="flex justify-end gap-3 text-sm sm:text-base">
              ...
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M11 5.44347V10.999H14.3333M21 10.999C21 16.5219 16.5229 20.999 11 20.999C5.47716 20.999 1 16.5219 1 10.999C1 5.47618 5.47716 0.999023 11 0.999023C16.5229 0.999023 21 5.47618 21 10.999Z" stroke="#555555" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </p>
            <p className="flex justify-end gap-3 text-sm sm:text-base">
              <span className="flex flex-col items-end gap-2.5">
                <span className="flex justify-end gap-3">
                  ...
                  <svg width="18" height="13" viewBox="0 0 18 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M2 7.26772L6.27419 11.5417L15.8169 1.99902" stroke="#555555" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </span>
                <span className="text-end mr-4 sm:mr-9 text-pretty text-xs sm:text-sm leading-6 sm:leading-7">
                  {course.goal}
                </span>
              </span>
            </p>
        ...
        <div className="w-full flex flex-col gap-4 md:gap-6">
          <div className="w-full rounded-xl overflow-hidden shadow-xl">
            <img src={course.thumbnail} alt="" className="w-full h-auto object-cover" />
          </div>
          <div className="w-full bg-white shadow-xl rounded-xl p-4 sm:p-5 flex flex-col gap-3 items-end">
            <h3 className="font-bold text-sm sm:text-base">:توضیحات کوتاه</h3>
            <p className="text-end text-pretty text-xs sm:text-sm">{course.shortDesc}</p>
          </div>
          <div className="min-h-[200px] bg-white shadow-xl p-4 sm:p-6 rounded-xl w-full flex flex-col gap-5 md:gap-7" dir="rtl">
            {renderEditorContent(course.longDesc)}
            <div className="py-4 sm:py-5 px-6 sm:px-14 border border-gray-500 rounded-xl flex flex-col items-center gap-4 sm:gap-5">
              <h3 className="font-black text-base sm:text-lg">سرفصل های دوره</h3>
                ...
                <div className="w-full flex flex-col" key={`${topic.head}-index-${index}`}>
                  ...
                  >
                    {openTopic.indexOf(index) > -1 ? (
                      ...
                      <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M14 11H11M11 11H8M11 11V8M11 11V14" stroke="black" stroke-width="1.5" stroke-linecap="round" />
                        <path d="M21 11C21 15.714 21 18.0711 19.5355 19.5355C18.0711 21 15.714 21 11 21C6.28595 21 3.92893 21 2.46447 19.5355C1 18.0711 1 15.714 1 11C1 6.28595 1 3.92893 2.46447 2.46447C3.92893 1 6.28595 1 11 1C15.714 1 18.0711 1 19.5355 2.46447C20.5093 3.43821 20.8356 4.80655 20.9449 7" stroke="black" stroke-width="1.5" stroke-linecap="round" />
                      </svg>
                    )}
                    {topic.head}
            ...
            {course.questions.map((question, index) => (
              <div className="" key={`${question.question}-index-${index}`}>
                <h4 className="font-bold mb-2 text-sm sm:text-base">{question.question}</h4>
                <p className="text-xs sm:text-sm font-light border-b pb-3">{question.answer}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {course.related && course.related.length > 0 && (
        <div className="w-full bg-white shadow-xl py-6 sm:py-8 md:py-10 px-4 sm:px-6 md:px-10 rounded-xl flex flex-col" dir="rtl">
          <h1 className="text-base sm:text-lg md:text-xl font-bold mb-4 sm:mb-6 md:mb-10">دوره‌های مشابه</h1>
