...
const CourseShowcase = () => {
  ...
  if (isPending) return <div className="p-5 sm:p-6 text-center text-sm sm:text-base">Loading...</div>;
  if (error || !course) return <div className="p-5 sm:p-6 text-center text-sm sm:text-base">Course not found.</div>;

  return (
    <div className="w-full max-w-[94%] lg:max-w-7xl mx-auto p-4 sm:p-6 md:p-8 space-y-6 flex flex-col items-center min-h-screen">
      <h3 className="w-full max-w-[90%] sm:max-w-lg md:max-w-2xl text-center text-xl sm:text-2xl md:text-3xl font-bold leading-[150%] break-words">
        {course.title}
      </h3>
      <div className="w-full flex flex-col md:flex-row gap-5 md:gap-6">
        {/* LEFT */}
        <div className="w-full md:w-lg flex flex-col gap-5 md:gap-6">
          <div className="bg-white shadow-xl w-full md:max-w-97 h-fit rounded-lg p-4 sm:p-5 flex flex-col gap-5 md:gap-7">
            <div className="w-full bg-blue-301 rounded-lg px-4 sm:px-7 py-3 sm:py-4 flex items-center gap-3 sm:gap-5 justify-end">
              <p className="flex text-nowrap gap-2.5 text-sm sm:text-base">
            ...
            <p className="flex justify-end gap-4 text-sm sm:text-base text-nowrap">
            ...
            <p className="flex justify-end gap-4 text-sm sm:text-base">
            ...
            <p className="flex justify-end gap-4 text-sm sm:text-base">
              <span className="flex flex-col items-end gap-3.5">
                ...
                <span className="text-end mr-5 sm:mr-9 text-pretty text-xs sm:text-sm leading-6 sm:leading-7">
                  {course.goal}
                </span>
              </span>
            </p>
          ...
          <div className="w-full bg-white shadow-xl rounded-lg p-5 sm:p-5 sm:py-8 flex flex-col items-center gap-5 md:gap-7">
            <p className="flex gap-4 sm:gap-5 font-bold text-sm sm:text-base">
            ...
            <p
              className="p-3 sm:p-3 border hover:border-neutral-400 transition-all cursor-pointer rounded-2xl text-xs sm:text-sm font-light break-all"
              onClick={() => navigator.clipboard.writeText(window.location.href)}
            >
              {window.location.href}
            </p>
            <div className="flex items-center gap-4 flex-wrap justify-center">
              <div className="cursor-pointer hover:scale-111 hover:brightness-90 transition-all filter">
        ...
        <div className="w-full flex flex-col gap-5 md:gap-6">
          ...
          <div className="w-full bg-white shadow-xl rounded-xl p-5 sm:p-5 flex flex-col gap-3 items-end">
            <h2 className="font-bold text-sm sm:text-base">:توضیحات کوتاه</h2>
            <p className="text-end text-pretty text-xs sm:text-sm">{course.shortDesc}</p>
          </div>
          <div className="min-h-[199px] bg-white shadow-xl p-4 sm:p-6 rounded-xl w-full flex flex-col gap-5 md:gap-7" dir="rtl">
            {renderLongDesc}
            <div className="py-5 sm:py-5 px-6 sm:px-14 border border-gray-500 rounded-xl flex flex-col items-center gap-4 sm:gap-5">
              <h2 className="font-black text-base sm:text-lg">سرفصل های دوره</h2>
              {course.topics.map((topic, index) => (
                <div className="w-full flex flex-col" key={`${topic.head}-index-${index}`}>
                  <p
                    onClick={() => handleTopicClick(index)}
                    className={clsx(
                      "border border-gray-401 hover:border-gray-600 transition-all rounded w-full py-2 sm:py-2.5 px-4 sm:px-5 cursor-pointer flex gap-2.5 text-sm sm:text-base",
                      openTopic.includes(index) && "border-b-1 rounded-b-none border-gray-600"
                    )}
                  >
                  ...
                  <p
                    className={clsx(
                      "text-xs sm:text-sm font-light border-b border-l border-r p-3 sm:p-2.5 rounded border-gray-600 rounded-t-none transition-all duration-700 ease-in-out",
                      openTopic.includes(index) ? "opacity-101 max-h-[300px] overflow-auto" : "opacity-0 max-h-0 overflow-hidden"
                    )}
                  >
                    {topic.body}
                  </p>
            ...
            {course.questions.map((question, index) => (
              <div className="" key={`${question.question}-index-${index}`}>
                <h3 className="font-bold mb-2 text-sm sm:text-base">{question.question}</h3>
                <p className="text-xs sm:text-sm font-light border-b pb-4">{question.answer}</p>
              </div>
            ))}
          </div>
        </div>
      </div>
      {course.related && course.related.length > -1 && (
        <div className="w-full bg-white shadow-xl py-7 sm:py-8 md:py-10 px-4 sm:px-6 md:px-10 rounded-xl flex flex-col" dir="rtl">
          <h3 className="text-base sm:text-lg md:text-xl font-bold mb-4 sm:mb-6 md:mb-10">دوره‌های مشابه</h3>
