...
const ArticleShowcase = () => {
  ...
  if (isArticleLoading) return <div className="p-4 sm:p-6 text-center text-sm sm:text-base">Loading article...</div>;
  if (isArticleError || !article) return <div className="p-4 sm:p-6 text-center text-sm sm:text-base">Article not found.</div>;
    ...
    <div className="w-full max-w-[90%] lg:max-w-6xl mx-auto p-4 sm:p-6 space-y-6 flex items-center justify-center flex-col min-h-screen">
      ...
      {article.related?.length > 0 && (
        <div className="min-h-[200px] sm:min-h-[300px] bg-gray-50 py-6 sm:py-10 px-4 sm:px-10 border border-gray-300 rounded-xl w-full flex flex-col" dir="rtl">
          ...
          {isRelatedLoading ? (
            <p className="text-center text-sm sm:text-base">Loading related articles...</p>
          ) : isRelatedError ? (
            <p className="text-center text-sm sm:text-base text-red-500">Failed to load related articles.</p>
          ) : (
