...
import type { CounselShort, Counsel } from "types/counsel";
...
const fetchCounsel = async (id: string): Promise<Counsel> => {
  const res = await fetch(`/api/counsel/${id}`);
  const data = await res.json();
  return data.counsel;
};

const fetchRelatedCounsels = async (relatedIds: string[]): Promise<CounselShort[]> => {
  const counsels = await Promise.all(
    relatedIds.map(async (relatedId) => {
      const res = await fetch(`/api/counsels/short/${relatedId}`);
      const data = await res.json();
      return data.counselObject;
    })
  );
  return counsels;
};
...
const CounselShowcase = () => {
  const { id } = useParams<{ id: string }>();
  const queryClient = useQueryClient();

  const {
    data: counsel,
    isLoading: isCounselLoading,
    isError: isCounselError,
  } = useQuery<Counsel>({
    queryKey: ["counsel", id],
    queryFn: () => fetchCounsel(id!),
    enabled: !!id,
  });
  ...
  // @ts-ignore
  const isFavorited = currentUser?.favoriteCounsels?.some(a => a._id === id);

  const toggleFavoriteMutation = useMutation({
    mutationFn: async () => {
      const token = localStorage.getItem("token");
      const res = await fetch(`/api/user/favorite/counsel/${id}`, {
        method: "POST",
        headers: {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "خطا در افزودن مشاوره به علاقه‌مندی‌ها");
      return data;
    },
    onSuccess: () => {
      // @ts-ignore
  ...
  const {
    data: relatedCounsels,
    isLoading: isRelatedLoading,
    isError: isRelatedError,
  } = useQuery<CounselShort[]>({
    queryKey: ["relatedCounsels", counsel?.related],
    queryFn: () => fetchRelatedCounsels(counsel?.related || []),
    enabled: !!counsel?.related?.length,
  });
          ...
          return (
            ...
            >
              <img
                ...
                alt={block.data.caption || "Counsel Image"}
  ...
  return (
    <div className="relative min-h-screen bg-gray-50">
      <Helmet>
        <title>{counsel.title} - modirfarda</title>
        <meta name="description" content={counsel.description || "خواندن مشاوره در مورد موضوعات مختلف در مدیر فردا"} />
        <meta property="og:title" content={counsel.title} />
        <meta property="og:description" content={counsel.description || "خواندن مشاوره در مورد موضوعات مختلف در مدیر فردا"} />
        <meta property="og:type" content="counsel" />
        <meta property="og:image" content={counsel.thumbnail} />
        <link rel="canonical" href={`https://modirfarda.com/counsel/${id}`} />
      </Helmet>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
        ...
        <motion.div
          ...
          dir="rtl"
        >
          {renderEditorContent(counsel.content)}
        </motion.div>

        {/* Related Counsels */}
        {counsel.related?.length > 0 && (
          <motion.div
            className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-200"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            dir="rtl"
          >
            <h2 className="text-2xl font-bold text-gray-900 mb-6">مشاوره‌های مشابه</h2>
            {isRelatedLoading ? (
              <p className="text-center text-lg text-gray-700">Loading related counsels...</p>
            ) : isRelatedError ? (
              <p className="text-center text-lg text-red-600">Failed to load related counsels.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {relatedCounsels?.map((relatedCounsel, index) => (
                  <motion.div
                    key={`relatedCounsel-${index}`}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5, delay: index * 0.1 }}
                  >
                    <CounselCard counsel={relatedCounsel} />
                  </motion.div>
                ))}
              </div>
            )}
...
export default CounselShowcase;