...
export default function AddCounselForm() {
  ...
  const { data: allCounsels = [] } = useQuery<CounselShort[]>({
    queryKey: ['counsels'],
    queryFn: () => axios.get('/api/counsels').then((res) => res.data),
  });
  ...
  useEffect(() => {
    const editor = new EditorJS({
      ...
      placeholder: 'مشاوره خود را بنویسید...',
      i18n: {
        direction: 'rtl'
      }
    });
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      return authAxios.post('/counsels', {
        title,
        description,
    ...
    onSuccess: () => {
      toast.success('مشاوره منتشر شد!');
    },
    onError: (error: any) => {
      console.error('خطا در انتشار مشاوره:', error);
      toast.error('انتشار مشاوره ناموفق بود :(');
    },
  });
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 space-y-6 bg-white shadow-lg rounded-3xl border border-gray-200" dir='rtl'>
      ...
      <div className="text-right pt-4">
        ...
        >
          {submitMutation.isPending ? 'در حال انتشار...' : 'انتشار مشاوره'}
        </button>
      </div>
    </div>
  );
